...
handleEvent (AwsEvent { queryStringParameters: { query, from, to } }) =
  ...
  downloadBook' book@(L.Book { extension }) = do
...
convertFile (L.Book book@{ downloaded, extension, title }) = do
...
sendEmail mailjetUser fileName file = do
...
getDownloadFileUrl preDownloadUrl =
  unsafePartial
    $ do
